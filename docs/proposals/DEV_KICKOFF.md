...
1. **Plan** — `docs/plans/multi-tenant-pauses-and-catalog.md`. This is your turn-by-turn for the work. Read it end-to-end before starting Phase 1.
2. **Proposal** — `docs/proposals/multi-tenant-pauses-and-catalog.md`. The "why" and the decisions already made. Read for context; the plan is the actionable document.
...
- Any other items from `docs/deferred-items.md`.
...
Phase 7 is the `/project-docs:finalize-branch` workflow. Single-commit squash (Strategy A), session doc in `docs/sessions/2026-05-DD-pause-table-rename-and-catalog-factory-session.md` (use the actual publish date), then merge to `develop`. `main` catches up via the release-please cycle.

## One-line summary for context cards

> Renaming `pauses` → `pauseTable` everywhere (breaking, alpha-window only), adding `createElevenLabsCatalog(apiKey)` factory, two README callouts, publishing as `0.2.0-alpha.0`. Both alpha consumers have acked. Plan: `docs/plans/multi-tenant-pauses-and-catalog.md`.
