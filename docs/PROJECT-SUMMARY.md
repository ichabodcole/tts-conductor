...
The defining design decision is a provider-pluggable contract: `@tts-conductor/core` knows nothing about specific TTS vendors, and concrete adapters implement a small `TtsProvider` interface. The first (and currently only) adapter is `@tts-conductor/provider-elevenlabs`, which wraps the official ElevenLabs JS SDK.
...
3. Run tests: `pnpm test` (or `pnpm --filter @tts-conductor/core test` to target one package)
...
- **The point of this project is the contract, not the adapter.** `@tts-conductor/core` was deliberately designed to be vendor-agnostic; the ElevenLabs package is one concrete proof of that contract. The interesting code is in `provider.ts`, `factory.ts`, `conductor.ts`, and `operations.ts`.
